...
;	submitted, 1991. This paper was presented as XXVIII COSPAR, The Hague,
;	STP I.2-P1, 1990.
...
;						40		daily flux total
;
;	K. Tobiska              Baseline version: 7-31-90      Rev: 4-24-91
;************************************************************************

moddays=1					; number of modeled days
yyddd=0.
c=fltarr(12,52)					; array of model coefficients
indfile=4 & index=[0,3750,365,8036]		; file sizes
indices=fltarr(10,index(indfile-1))		; array of proxy values
dates=fltarr(16072)				; array of dates available
...
temp1=fltarr(39)&temp2=temp1
temp=fltarr(6)

openr,1,'euv91coef.txt'				; get the coefficients
  line=' '
  for i=0,2 do readf,1,line
  readf,1,c & sizec=size(c)
openr,21,'euv91index1.dat'			; get the indices
  for i=0,1 do readf,21,line
  ii1=fltarr(10,3921)
  readf,21,ii1 
openr,22,'euv91index2.dat'			; get the indices
  for i=0,1 do readf,22,line
  ii2=fltarr(10,3750)
  readf,22,ii2
openr,23,'euv91index3.dat'			; get the indices
  for i=0,1 do readf,23,line
  ii3=fltarr(10,365)
  readf,23,ii3
close,1 & close,21,22,23
  indices(0,0)=ii1(0:4,*) & indices(0,3921)=ii1(5:9,*)
  indices(0,7842)=ii2(0:4,1:194) & indices(5,0)=ii2(0:4,195:*)
  indices(5,3555)=ii2(5:9,*) & indices(5,7305)=ii3(0:4,*)
  indices(5,7670)=ii3(5:9,*) 
  dates(0)=transpose(indices(0,*))
  dates(index(indfile-1))=transpose(indices(5,*))
  lya(0)=transpose(indices(1,*))&lya(index(indfile-1))=transpose(indices(6,*))
  he1(0)=transpose(indices(2,*))&he1(index(indfile-1))=transpose(indices(7,*))
  f10(0)=transpose(indices(3,*))&f10(index(indfile-1))=transpose(indices(8,*))
  f81(0)=transpose(indices(4,*))&f81(index(indfile-1))=transpose(indices(9,*))
moddays=15995
...
getdate:
yyddd=47045
read,' What is the starting date (yyddd: 47045-90343)? ',yyddd
  ind1=where(dates eq yyddd)			; find the starting date
  if (ind1(0) eq -1) then begin
    answer=' ' & inpt=0.
    read,' Would you like to input specific values for a date? ',answer
    if (answer eq 'y') then begin		; input the specific data
      read,' Date        ',inpt & dates(0)=inpt	; date outside of range
      read,' Lyman-alpha ',inpt & lya(0)=inpt	; Ly-a daily value
      read,' Helium I    ',inpt & he1(0)=inpt	; He I daily value
      read,' F10.7 daily ',inpt & f10(0)=inpt	; F10.7 daily value
      read,' F10.7 81-d  ',inpt & f81(0)=inpt	; F10.7 81 day mean
      ind1(0)=0
    end
    if (answer eq 'n') then goto,getdate	; date out of range, try again
  end
...
answer='n'
goto,groups
read,' Scale any wavelength bins? ',answer
if (answer eq 'y') then begin
  numbin=0
  read,' How many bins (out of 0-'+strtrim(sizec(2)-1,2)+')? ',numbin
  for i=0,numbin-1 do begin
    bin=0 & scale=0. & typ=['(Chromo)','(Corona)']
    read,' Which bin? ',bin
    read,' Scale factor? ',scale
    model(bin,0) = model(bin,*)*scale
    print,' Scaled '+strtrim(c(0,bin),2)+' '+strtrim(c(1,bin),2)+' by x '+$
	strtrim(scale,2)+' '+typ(fix(c(2,bin))-1)
  end
end

groups:		;print,'Making 5 nm bins and 39 wavelength groups'
